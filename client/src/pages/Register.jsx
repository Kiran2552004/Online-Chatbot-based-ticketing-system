...
const Register = () => {
  ...
  const fields = [
    ...
    { label: 'Phone (Optional)', name: 'phone', type: 'tel', placeholder: 'Enter your phone number', required: false },
  ];
    ...
    <AnimatedBackground variant="purple" className="flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="relative p-8 pb-6 border-b border-white/8" style={{ background: 'linear-gradient(135deg,rgba(139,92,246,0.12) 0%,rgba(236,72,153,0.06) 100%)' }}>
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-20" style={{ background: 'radial-gradient(ellipse,rgba(139,92,246,0.2) 0%,transparent 70%)', filter: 'blur(16px)' }} />
          <div className="relative z-10 text-center">
            ...
            <p className="text-purple-300 text-sm mt-1">Join us and start exploring</p>
          </div>
        ...
        <div className="p-8 space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {fields.map((f) => (
              <motion.div key={f.name} variants={slideUp}>
                <label className="block text-xs font-bold uppercase tracking-widest text-purple-400 mb-2">{f.label}</label>
            ...
            <motion.button type="submit" disabled={loading}
              ...
              className="relative w-full py-3.5 rounded-xl font-bold text-white overflow-hidden mt-2"
              style={{
              ...
              <span className="relative z-10 flex items-center justify-center gap-2">
                {loading ? (<><motion.span animate={{ rotate: 360 }} transition={{ duration: 0.9, repeat: Infinity, ease: 'linear' }} className="inline-block w-4 h-4 border-2 border-white/25 border-t-white rounded-full" /> Creating account…</>) : '🚀 Create Account'}
              </span>
            </motion.button>
          </form>
          ...
          <div className="space-y-3 pt-1">
            <p className="text-center text-purple-400 text-sm">
              ...
              <Link to="/login" className="text-purple-300 hover:text-white underline underline-offset-2 font-semibold transition-colors">Login here</Link>
            </p>
